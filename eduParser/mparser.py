...
class ProfileParser():
    def __init__(self,employee,text=None,max_line=32,min_char=2,lines=None, 
        set_attr_hook=None,force_inline=False,ignore=set(),force_email=False,force_tel=False):
        self.text = text
    ...
    # ref: https://gist.github.com/dideler/5219706
    def parse_email(self,line):
        ...
        return (email[0] for email in re.findall(regex, line) if not email[0].startswith('//'))

    def parse_tel(self,line):
        # 电话号码与手机号码同时验证:(^(\d{3,4}-)?\d{7,8})$|(13[0-9]{9}) 
        regex = re.compile(("(^(\d{3,4}-)?\d{7,8})$|(13[0-9]{9})"))
        return (tel[0] for tel in re.findall(regex, line))
    ...
    def parse_inline(self, line):
        ...
        # 根据预定的symbol推测字段和值
        line_piped = line.lower()
        for symbol, name in self.symbols.items():
            if name in self.ignore:
                continue
            idx = line_piped.find(symbol)
            if idx != -1:
                pos = idx + len(symbol)
                # 对于‘电子邮箱’，‘邮箱’两个关键串都可以匹配，为了避免可能的冗余需要进行去重
                if not (pos in index_name_symbols):
                    index_name_symbols[pos] = [name, symbol]
        ...
        for pos, index in enumerate(indexes):
            name = index_name_symbols[index][0]
            value = None
            if pos < (indexes_len-1):
                next_index = indexes[pos+1]
                symbol = index_name_symbols[next_index][0]
                end_idx = indexes[pos+1]-len(symbol)
                if end_idx >= index:
        ...
        if self.force_email:
            emails = []
            for email in self.parse_email(line):
                emails.append(email)
            if len(emails) != 0:
                name_values['email'] = ';'.join(emails)
        ...
        if self.force_tel:
            tels = []
            for tel in self.parse_tel(line):
                tels.append(tel)
            if len(tels) != 0:
                name_values['tel'] = ';'.join(tels)
        return None, name_values

    def set_next_line(self,name,value):
        line = value.replace(',','，')
        if not name in set(['profile']):
            line = line.replace('：','')
            line = line.replace(':','')
        ...
        for count,line in enumerate(self.lines):
            ...
            if to_parse_value:
                ...
                if self.check_symbols(line):
                    ...
                    if self.set_next_line(to_parse_value,line):
                        to_parse_value = None
                        #print "self.employee.email1 -> " + self.employee.email
            ...
            if not to_parse_value:
                for name, value in name_values.items():
                    self.set_next_line(name,value)
                    #print "self.employee.email2 -> " + self.employee.email
