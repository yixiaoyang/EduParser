...
# @output:输出employee
def handler(tag):
    name_div = tag.find_all('a')
    zc_div = tag.find_all('td',class_='zc')
    research_div = tag.find_all('td',class_='major')
    ...
    return Employee(name=name,title=title, research=research,url=url)
...
def profile_handler(doc, name, url, path):
    ...
    divs = soup.find_all(name="div", attrs={"id":"main"}, limit=1)
